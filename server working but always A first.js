...
function initializeGame() {
    ...
    gameState.round = 1;
    // Set the round start player based on round number for FIRST TRICK only: A=0, B=1, C=2, D=3
    // Round 1->A, Round 2->B, Round 3->C, Round 4->D, Round 5->A, etc.
    gameState.roundStartPlayer = (gameState.round - 1) % 4;
...
function completeRound() {
    ...
    if (gameState.round >= 13) {
        ...
        setTimeout(() => {
            gameState.round++;
            // Set first trick leader for new round based on fixed rotation: A->B->C->D->A...
            // Round 1->A(0), Round 2->B(1), Round 3->C(2), Round 4->D(3), Round 5->A(0)...
            gameState.roundStartPlayer = (gameState.round - 1) % 4;
            gameState.biddingStartPlayer = gameState.roundStartPlayer;
